...
/*   Updated: 2024/06/15 22:33:54 by anfi             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_if_foe_can_move(t_data *data, int y, int x)
{
	...
	if (ft_strchr("1ECF", data->map->map[y][x - 1]))
	{
		...
		x = temp_x;
		if (ft_strchr("1EC", data->map->map[y][x + 1]) != NULL)
		...
		else if (data->map->map[y][x + 1] == 'F')
		{
			while ( x < data->map->col_num && data->map->map[y][x + 1] == 'F')
			{
				if (!ft_strchr("1EC", data->map->map[y][x + 1]))
					return (1);
				else if (ft_strchr("0", data->map->map[y][x + 1]) && temp_x - x > 1)
				{
					while (--x + 1 >= temp_x)
						data->foe_info[data->map->ff_map[y][x]].dir = RIGHT;
					move_many_foes_right(data, y, x, temp_x);
				}
				x++;
			}
			return (change_foe_direction(data, y, x, PERMANENT_STILL), 0);
...
int		move_foe_right(t_data *data, int y, int x, int n)
{
	...
	else if (data->map->ff_map[y][x + 1] != -1)
	{
		...
		else if (data->foe_info[data->map->ff_map[y][x + 1]].dir == RIGHT)
			return (move_many_foes_right(data, y, x, n));
	}
	else
...
int		move_foe(t_data *data, int y, int x, int n)
{
	...
	if ( data->movements <= 1 && (data->foe_info[n].dir == PERMANENT_STILL || check_if_foe_can_move(data, y, x) == 0))
		return (0);
